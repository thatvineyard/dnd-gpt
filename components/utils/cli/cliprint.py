...
def cli_print_info(message: str, prefix: str = "", message_style: CliStyle = CliStyle.DEFAULT, prefix_style: CliStyle = CliStyle.BLUE):
    if(_level <= logging.INFO):
        print(f'{_wrap_in_style(prefix, prefix_style)}{_wrap_in_style(message, message_style)}')
